...
class IH_BVP_solver:
    ...
    def solve_BVP(self, Ra=460, Pr=1, epsilon=1e-4, n_rho=3, r=2, nz=256, use_therm=True):
        ...
        if self.rank < 10000:
            ...
            #atmosphere.problem.substitutions['rho1_z_C'] = '(-1/w_IVP)*(rho0_tot*Div_u_IVP + w_IVP*dz(rho0_tot))'
            atmosphere.problem.substitutions['rho1_z_L'] = 'dz(rho1)'
            atmosphere.problem.substitutions['rho1_z_C'] = '0'
            ...
            atmosphere.problem.substitutions['R_NL_HSB'] = '(R_rho_gradT + R_T_gradrho)'
            if use_therm:
                atmosphere.problem.substitutions['R_IVP_HSB'] = '(rho0*T1_z_IVP + T0 * dz(rho1_IVP) + rho1_IVP*T0_z + T1_IVP * dz(rho0) +'\
                                                                ' rho1_IVP * T1_z_IVP + T1_IVP * dz(rho1_IVP) + rho1_IVP*g + T1_IVP * rho1_z_C)'
                atmosphere.problem.substitutions['L_IVP_HSB'] = '( rho1_IVP * T1_z + T1_IVP * rho1_z_L + rho1 * T1_z_IVP + T1 * dz(rho1_IVP)  )'
            else:
            ...
            atmosphere.problem.add_bc('left(rho1) = 0') 
    ...
    #            plt.close()
        ...
        self.comm.Barrier()
        for v in VARS.keys():
            self.comm.Allreduce(return_dict[v], return_dict[v], op=MPI.SUM)
